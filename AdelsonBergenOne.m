...
% numDots  = 1;
% 
% [imgCols, imgRows] = meshgrid(1:imgSizeX, 1:imgSizeY);
% 
% imgCenter = [imgSizeX/2, imgSizeY/2]; % Image center in x and y
% 
% circleRadius = 10;
% 
% stim = (imgRows - imgCenter(2)).^2 + (imgCols - imgCenter(1)).^2 <= circleRadius.^2; % Define circle
% % imshow(stim);
% colormap([0.25, 0.25, 0.25 ; 0.75, 0.75, 0.75]); % [Background, foreground]
% 
% % Create frames and make the dot move
% scr.frameDuration        = 17;     % [msec] approximated!! 
% scr.pixelsPerFrame       = 5;
